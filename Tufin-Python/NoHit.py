...
def Juniper(TufinIP, userpassencoded, deviceID, deviceName, curtime, daysreq):
    # Disable example:
    #deactivate security policies from-zone <from_zone> to-zone <to_zone> policy <policy_name>
    print("[D] Generating Juniper scripts for bulk policy disable & enable.")
    from_zone = ""
    to_zone = ""
    policy_name = ""
    txtPathdisable = "./{}/disable-rule-{}.txt".format(deviceName, curtime)
    txtPathenable = "./{}/enable-rule-{}.txt".format(deviceName, curtime)
    url = "https://{}/securetrack/api/rule_search/{}?search_text=lasthitgreater:{} log.isempty:false rule.isdisabled:false".format(TufinIP, deviceID, daysreq)
    headers = {
    ...
    for child in tree:
        if child.tag == "rule":
            for y in child:
                ...
                if y.tag == "name":
                    policy_name = y.text
            f = open(txtPathdisable, "a+")
            print("File Name: {}".format(txtPathdisable))
            print("Writing disable script for {}".format(policy_name))
            f.write("deactivate security policies from-zone {} to-zone {} policy {}\n".format(from_zone, to_zone, policy_name))
            f.close()
            f = open(txtPathenable, "a+")
            print("File Name: {}".format(txtPathenable))
            print("Writing enable script for {}".format(policy_name))
            f.write("activate security policies from-zone {} to-zone {} policy {}\n".format(from_zone, to_zone, policy_name))
            f.close()
        else:
            pass
    print("Disable script file: {}\nPaste it on Juniper using CLI to disable No Hit rules exceed {} days.".format(txtPathdisable, daysreq))
    print("To re-enable rules, use file: {}\nPaste it on Juniper using CLI to enable rules again.".format(txtPathenable))
# ------------------------------------------------------------------------------

# Main Function ----------------------------------------------------------------
def OptimizeNoHit(TufinIP, userpassencoded, deviceID, deviceName, deviceType, curtime):
    # Model define
    ...
    daysreq = int(input("How many days require for No Hit rules?\n(Ex: '30' for more than 30 days, '7' for 7 days, ...):"))

    # Export with No Hit polices in all Device
    url = "https://{}/securetrack/api/rule_search/export?search_text=lasthitgreater:{}".format(TufinIP, daysreq)
    headers = {
        'Authorization': "Basic %s" %userpassencoded
        }
    print("[D] Exporting all No Hit policy!")
    response = requests.get(url, headers=headers, verify=False)
    ...
    for x in range(0, len(deviceID)):
        ...
        if deviceType[x] in CheckPoint_model:
            print("No supported CLI.\n Please disable manually via Tufin SecureChange Ticket!\n- - - - - - - - - - - - - - - - - - - - -")
        elif deviceType[x] in Cisco_model:
            print("No supported CLI.\n Please disable manually via Tufin SecureChange Ticket!\n- - - - - - - - - - - - - - - - - - - - -")
        elif deviceType[x] in Juniper_model:
            Juniper(TufinIP, userpassencoded, deviceID[x], deviceName[x], curtime, daysreq)
